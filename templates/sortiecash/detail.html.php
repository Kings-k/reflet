...
<div class="row">
    <div class="col-md-8 mb-4">
        <div class="card shadow">
            ...
            <div class="card-body">
                <form action="/sortiecash/<?= $sortie->getId() ?>" method="post">
                    <div class="form-outline my-4">
                        <input type="number" name="montant" id="montant" class="form-control form-control-lg" placeholder="Montant en $" value="<?= $sortie->getMontant() ?>" required autofocus />
                        <label class="form-label" for="montant">Montant</label>
                    </div>
                    <div class="form-outline my-4">
                        <input type="text" name="piece" id="piece" class="form-control form-control-lg" value="<?= $sortie->getPieceJustificative() ?>" />
                        <label class="form-label" for="piece">Pièce Justificative</label>
                    </div>
                    <div class="form-outline my-4">
                        <input type="text" name="motif" id="motif" class="form-control form-control-lg" value="<?= $sortie->getMotif() ?>" />
                    ...
                    <div class="form-group my-4">
                        <select name="typesortie" id="typesorite" class="select" data-filter="true">
                            <?php foreach ($typesSortie as $type) : ?>
                                <option value="<?= $type->getId() ?>"<?= $sortie->getTypeSortie()->getId() === $type->getId() ? ' selected' : '' ?>><?= $type->getCompte() ?></option>
                            <?php endforeach ?>
                        </select>
